  ...
  stages {
    ...
    stage("Set tag custom bash"){
      steps{
        script{
          env.EXT_RELEASE = sh(
            script: ''' curl -sX GET http://ppa.launchpad.net/jcfp/ppa/ubuntu/dists/xenial/main/binary-amd64/Packages.gz | gunzip |grep -A 7 -m 1 'Package: sabnzbdplus' | awk -F ': ' '/Version/{print $2;exit}' | sed 's/$/-u/' ''',
            returnStdout: true).trim()
    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              ...
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=unstable -v ${TEMPDIR}:/ansible/readme linuxserver/doc-builder:latest
              if [ "$(md5sum ${TEMPDIR}/${LS_REPO}/Jenkinsfile | awk '{ print $1 }')" != "$(md5sum Jenkinsfile | awk '{ print $1 }')" ] || [ "$(md5sum ${TEMPDIR}/${CONTAINER_NAME}/README.md | awk '{ print $1 }')" != "$(md5sum README.md | awk '{ print $1 }')" ]; then
                ...
                git --git-dir ${TEMPDIR}/repo/${LS_REPO}/.git checkout -f unstable
      ...
      steps {
        withCredentials([
          ...
          sh '''#! /bin/bash
             echo $DOCKERPASS | docker login -u $DOCKERUSER --password-stdin
             '''
          sh "docker tag ${IMAGE}:${META_TAG} ${IMAGE}:unstable"
    ...
    stage('Docker-Push-Multi') {
      ...
      steps {
        withCredentials([
          ...
          sh "docker tag ${IMAGE}:amd64-${META_TAG} ${IMAGE}:amd64-unstable"
          sh "docker tag ${IMAGE}:arm32v6-${META_TAG} ${IMAGE}:arm32v6-unstable"
          sh "docker tag ${IMAGE}:arm64v8-${META_TAG} ${IMAGE}:arm64v8-unstable"
          sh "docker push ${IMAGE}:amd64-${META_TAG}"
          sh "docker push ${IMAGE}:arm32v6-${META_TAG}"
          sh "docker push ${IMAGE}:arm64v8-${META_TAG}"
          sh "docker push ${IMAGE}:amd64-unstable"
          sh "docker push ${IMAGE}:arm32v6-unstable"
          sh "docker push ${IMAGE}:arm64v8-unstable"
          sh "docker manifest push --purge ${IMAGE}:unstable || :"
          sh "docker manifest create ${IMAGE}:unstable ${IMAGE}:amd64-unstable ${IMAGE}:arm32v6-unstable ${IMAGE}:arm64v8-unstable"
          sh "docker manifest annotate ${IMAGE}:unstable ${IMAGE}:arm32v6-unstable --os linux --arch arm"
          sh "docker manifest annotate ${IMAGE}:unstable ${IMAGE}:arm64v8-unstable --os linux --arch arm64 --variant v8"
    ...
    stage('Github-Tag-Push-Release') {
      ...
      steps {
        ...
        -d '{"tag":"'${EXT_RELEASE_CLEAN}'-pkg-'${PACKAGE_TAG}'-ls'${LS_TAG_NUMBER}'",\
             "object": "'${COMMIT_SHA}'",\
             "message": "Tagging Release '${EXT_RELEASE_CLEAN}'-pkg-'${PACKAGE_TAG}'-ls'${LS_TAG_NUMBER}' to unstable",\
             "type": "commit",\
