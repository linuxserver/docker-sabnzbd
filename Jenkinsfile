  ...
  stages {
    ...
    stage("Set ENV PR build"){
      ...
      steps {
        script{
          ...
          if (env.MULTIARCH == 'true') {
            env.CI_TAGS = 'amd64-unstable-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-pr-' + env.PULL_REQUEST + '|arm64v8-unstable-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-pr-' + env.PULL_REQUEST
          } else {
    ...
    stage("Scarf.sh package registry"){
      ...
      steps{
        sh '''#! /bin/bash
              set -e
              PACKAGE_UUID=$(curl -X GET -H "Authorization: Bearer ${SCARF_TOKEN}" https://scarf.sh/api/v1/organizations/linuxserver-ci/packages | jq -r '.[] | select(.name=="linuxserver/sabnzbd") | .uuid')
              if [ -z "${PACKAGE_UUID}" ]; then
                echo "Adding package to Scarf.sh"
                curl -sX POST https://scarf.sh/api/v1/organizations/linuxserver-ci/packages \
                  -H "Authorization: Bearer ${SCARF_TOKEN}" \
                  -H "Content-Type: application/json" \
                  -d '{"name":"linuxserver/sabnzbd",\
                       "shortDescription":"example description",\
                       "libraryType":"docker",\
                       "website":"https://github.com/linuxserver/docker-sabnzbd",\
                       "backendUrl":"https://ghcr.io/linuxserver/sabnzbd",\
                       "publicUrl":"https://lscr.io/linuxserver/sabnzbd"}' || :
              else
          ...
          retry(5) {
            sh '''#! /bin/bash
                  ...
                  for MANIFESTIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                    ...
                    docker manifest create ${MANIFESTIMAGE}:unstable ${MANIFESTIMAGE}:amd64-unstable ${MANIFESTIMAGE}:arm64v8-unstable
                    ...
                    docker manifest push --purge ${MANIFESTIMAGE}:arm32v7-unstable || :
                    docker manifest create ${MANIFESTIMAGE}:arm32v7-unstable ${MANIFESTIMAGE}:amd64-unstable
                    docker manifest push --purge ${MANIFESTIMAGE}:arm32v7-unstable
                    docker manifest push --purge ${MANIFESTIMAGE}:unstable
                    docker manifest push --purge ${MANIFESTIMAGE}:${META_TAG} 
                    docker manifest push --purge ${MANIFESTIMAGE}:${EXT_RELEASE_TAG} 
